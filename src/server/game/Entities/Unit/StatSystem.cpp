...
float Player::GetMissPercentageFromDefence() const
{
    ...
    nondiminishing += (GetSkillValue(SKILL_DEFENSE) - GetMaxSkillValueForLevel()) * 0.5f; // wowwiki http://www.wowwiki.com/Hit#Hit_rating_cap says it's 0.5%, not 0.04%!
    diminishing += (int32(GetRatingBonusValue(CR_DEFENSE_SKILL))) * 0.5f;
...
void Player::UpdateParryPercentage()
{
    ...
    if (CanParry() && parry_cap[pclass] > 0.0f)
    {
        ...
        // Modify value from defense skill (only bonus from defense rating diminishes)
        nondiminishing += (GetSkillValue(SKILL_DEFENSE) - GetMaxSkillValueForLevel()) * 0.5f;
        diminishing += (int32(GetRatingBonusValue(CR_DEFENSE_SKILL))) * 0.5f;
