...
void Player::CreateWowarmoryFeed(uint32 type, uint32 data, uint32 item_guid, uint32 item_quality) {
    ...
    */
    if (GetGUIDLow() == 0)
    {
        sLog->outInfo(LOG_FILTER_TSCR,"[Wowarmory]: player is not initialized, unable to create log entry!");
        return;
    }
    if (type <= 0 || type > 3)
    {
        sLog->outInfo(LOG_FILTER_TSCR,"[Wowarmory]: unknown feed type: %d, ignore.", type);
        return;
    }
    if (data == 0)
    {
        sLog->outInfo(LOG_FILTER_TSCR,"[Wowarmory]: empty data (GUID: %u), ignore.", GetGUIDLow());
        return;
    }
